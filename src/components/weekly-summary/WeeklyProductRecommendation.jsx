...
const WeeklyProductRecommendation = () => {
  ...
  const getNewRecommendation = useCallback(async () => {
    ...
    try {
      ...
      // Mengalikan target harian dengan 7 untuk mendapatkan target mingguan
      const targetMingguan = {
        energy_kal: Number(kebutuhanHarian.energi) * 7,
        protein_g: Number(kebutuhanHarian.protein) * 7,
        fat_g: Number(kebutuhanHarian["lemak total"]) * 7,
        carbohydrate_g: Number(kebutuhanHarian.karbohidrat) * 7,
        fiber_g: Number(kebutuhanHarian.serat) * 7,
        sugar_g: Number(kebutuhanHarian.gula) * 7,
        sodium_mg: Number(kebutuhanHarian.garam) * 7,
      };

      // Menghitung total nutrisi mingguan
      const totalGizi = weeklyHistory.reduce((acc, item) => {
      ...
      // Log informasi nutrisi mingguan
      console.log("=== Informasi Nutrisi Mingguan ===");
      console.log("Target Mingguan:", targetMingguan);
      console.log("Total Konsumsi Mingguan:", totalGizi);

      // Menyiapkan payload sesuai format API
      const inputData = {
        konsumsi: totalGizi,
        target_harian: targetMingguan, // Menggunakan target mingguan
      };
      ...
      <div className="mb-4 md:mb-6 animate-fade-in animate-delay-100">
        <p className="text-xs md:text-sm text-gray-600">
          {loading
            ...
            : "Rekomendasi produk berdasarkan konsumsi gizi mingguan Anda"}
        </p>
      </div>
      ...
      {loading && (
        <div className="flex flex-col items-center justify-center p-8 md:p-12 animate-pulse">
          ...
          <p className="mt-4 text-sm md:text-base text-main font-medium">
            Sedang memuat rekomendasi mingguan...
          </p>
        </div>
      )}
            ...
            recommendation.gizi_fokus.length > 0 && (
              <div className="bg-yellow-50/80 backdrop-blur-sm p-4 md:p-6 rounded-xl md:rounded-2xl border border-yellow-200 shadow-sm hover:shadow-xl transition-all duration-300 group overflow-hidden">
                ...
                <p className="text-xs md:text-sm text-yellow-700 mb-4 animate-fade-in animate-delay-100">
                  ...
                  target mingguan Anda. Silakan penuhi kebutuhan gizi tersebut
            ...
            recommendation.rekomendasi.length > 0 && (
              <div className="bg-white/90 backdrop-blur-sm p-4 md:p-6 rounded-xl md:rounded-2xl border border-main/20 shadow-sm hover:shadow-xl transition-all duration-300 group overflow-hidden">
                ...
                <p className="text-xs md:text-sm text-gray-600 mb-4 animate-fade-in animate-delay-100">
                  ...
                  mingguan Anda. Semakin tinggi skor gizi, semakin sesuai produk
...
export default WeeklyProductRecommendation;
