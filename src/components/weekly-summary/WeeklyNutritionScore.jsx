  ...
  useEffect(() => {
    const calculateWeeklyScore = async () => {
      ...
      try {
        ...
        if (thisWeekScans.length === 0) {
          throw new Error(
            "Anda belum memiliki riwayat pemindaian minggu ini. Silakan lakukan pemindaian makanan terlebih dahulu."
          );
        }

        // Menghitung total nutrisi untuk minggu ini
        const totalGizi = thisWeekScans.reduce((acc, item) => {
        ...
        Object.keys(totalGizi).forEach((key) => {
          totalGizi[key] = totalGizi[key] / 7; // Bagi dengan 7 untuk mendapatkan rata-rata harian
        });
        ...
        // Menghitung skor gizi menggunakan API
      ...
      <h2 className="text-lg md:text-xl font-bold text-main mb-4 md:mb-6 flex items-center animate-float">
        ...
        <div>
          ...
          <div className="text-xs md:text-sm text-gray-500 mt-1">
            Kualitas asupan nutrisi minggu ini
          </div>
        </div>
      </h2>
          ...
          <div className="bg-white/90 backdrop-blur-sm p-4 md:p-6 rounded-xl md:rounded-2xl border border-gray-200/80 shadow-sm hover:shadow-xl transition-all duration-300 group overflow-hidden min-h-[320px] md:min-h-[420px] flex items-center justify-center">
            <div className="flex flex-col items-center">
              ...
              <p className="text-sm md:text-base text-gray-600 text-center max-w-sm font-medium px-2 md:px-0">
                {score >= 8
                  ? "Asupan gizi mingguan Anda sangat seimbang dan ideal"
                  : score >= 6
                  ? "Asupan gizi mingguan cukup baik, masih bisa ditingkatkan"
                  : "Perlu perbaikan dalam pola makan mingguan Anda"}
              </p>
              {score < 8 && (
                <button
                  onClick={() => {
...
export default WeeklyNutritionScore;
