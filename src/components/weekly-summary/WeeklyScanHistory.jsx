...
const WeeklyScanHistory = () => {
  ...
  useEffect(() => {
    const fetchWeeklyScans = async () => {
      try {
        ...
        const weeklyData = (data?.history || []).filter((item) => {
  ...
  const groupedScans = groupScansByDay(weeklyScans);
        ...
        {loading ? (
          ...
          <div className="bg-white/90 backdrop-blur-sm p-4 md:p-8 rounded-xl md:rounded-2xl border border-gray-200/80 shadow-sm">
            <p className="text-sm md:text-base text-gray-600 text-center">
              Belum ada data scan untuk minggu ini
            </p>
          </div>
        ) : (
          <>
...
export default WeeklyScanHistory;
