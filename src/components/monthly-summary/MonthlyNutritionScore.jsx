  ...
  useEffect(() => {
    const calculateMonthlyScore = async () => {
      ...
      try {
        ...
        if (thisMonthScans.length === 0) {
          throw new Error(
            "Anda belum memiliki riwayat pemindaian bulan ini. Silakan lakukan pemindaian makanan terlebih dahulu."
          );
        }

        // Menghitung total nutrisi untuk bulan ini
        const totalGizi = thisMonthScans.reduce((acc, item) => {
        ...
        // Hitung rata-rata harian untuk bulan ini
        const daysInMonth = endOfMonth.getDate();
        Object.keys(totalGizi).forEach((key) => {
          totalGizi[key] = totalGizi[key] / daysInMonth; // Bagi dengan jumlah hari dalam bulan ini
        });
      ...
      <h2 className="text-lg md:text-xl font-bold text-main mb-4 md:mb-6 flex items-center animate-float">
        ...
        <div>
          ...
          <div className="text-xs md:text-sm text-gray-500 mt-1">
            Kualitas asupan nutrisi bulan ini
          </div>
        </div>
      </h2>
          ...
          <div className="bg-white/90 backdrop-blur-sm p-4 md:p-6 rounded-xl md:rounded-2xl border border-gray-200/80 shadow-sm hover:shadow-xl transition-all duration-300 group overflow-hidden min-h-[320px] md:min-h-[420px] flex items-center justify-center">
            <div className="flex flex-col items-center">
              ...
              <p className="text-sm md:text-base text-gray-600 text-center max-w-sm font-medium px-2 md:px-0">
                {score >= 8
                  ? "Asupan gizi bulanan Anda sangat seimbang dan ideal"
                  : score >= 6
                  ? "Asupan gizi bulanan cukup baik, masih bisa ditingkatkan"
                  : "Perlu perbaikan dalam pola makan bulanan Anda"}
              </p>
              {score < 8 && (
                <button
                  onClick={() => {
...
export default MonthlyNutritionScore;
