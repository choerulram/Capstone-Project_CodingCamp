...
const MonthlyTotalNutritionSummary = () => {
  ...
  useEffect(() => {
    const fetchMonthlyNutrition = async () => {
      try {
        ...
        // Hitung total nutrisi untuk bulan ini
        const monthlyTotal = thisMonthScans.reduce(
          (acc, item) => {
        ...
        setTotalNutrition(monthlyTotal);
      } catch (err) {
        console.error("Error fetching monthly nutrition data:", err);
      }
    };
      ...
      <h2 className="text-lg md:text-xl font-bold text-main mb-4 md:mb-6 flex items-center animate-float">
        ...
        <div>
          <div className="text-lg md:text-xl font-bold text-main">
            Total Nutrisi Bulanan
          </div>
          <div className="text-xs md:text-sm text-gray-500 mt-1">
            Asupan gizi bulan ini
          </div>
        </div>
      </h2>
...
export default MonthlyTotalNutritionSummary;
