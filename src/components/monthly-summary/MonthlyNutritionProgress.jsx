...
const MonthlyNutritionProgress = () => {
  const { token } = useSelector((state) => state.auth);
  const [monthlyTarget, setMonthlyTarget] = useState({
    energi: 60000, // 2000 * 30 (rata-rata hari dalam sebulan)
    protein: 1800, // 60 * 30
    "lemak total": 2100, // 70 * 30
    karbohidrat: 9000, // 300 * 30
    serat: 750, // 25 * 30
    gula: 1500, // 50 * 30
    garam: 60000, // 2000 * 30
  });
    ...
    const fetchData = async () => {
      try {
        ...
        if (dailyData?.kebutuhan_harian) {
          const monthly = Object.entries(dailyData.kebutuhan_harian).reduce(
            (acc, [key, value]) => {
              acc[key] = value * 30; // menggunakan 30 hari sebagai rata-rata bulan
              return acc;
            },
            {}
          );
        ...
        // Ambil semua riwayat scan dan filter untuk bulan ini
        ...
        const totals = thisMonthScans.reduce(
          (acc, scan) => {
  ...
  const nutrients = [
    ...
    },
    {
      ...
      label: "Lemak",
      unit: "g",
      target: monthlyTarget["lemak total"],
    ...
    },
    {
      ...
      label: "Karbohidrat",
      unit: "g",
      target: monthlyTarget.karbohidrat,
...
export default MonthlyNutritionProgress;
