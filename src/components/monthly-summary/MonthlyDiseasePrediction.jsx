...
const MonthlyDiseasePrediction = () => {
  ...
  useEffect(() => {
    const predictDiseaseRisk = async () => {
      ...
      try {
        ...
        // Mengambil riwayat pemindaian bulan ini
        const historyData = await api.getAllScanHistory(token);
        const allScans = historyData.history || [];
        ...
        const monthlyHistory = allScans.filter((scan) => {
        ...
        // Menghitung total nutrisi bulanan dengan explicit type conversion
        ...
        // Hitung rata-rata harian berdasarkan jumlah hari dalam bulan ini
        const daysInMonth = endOfMonth.getDate();
        const avgNutrisi = {
          Calories: totalNutrisi.Calories / daysInMonth,
          Protein: totalNutrisi.Protein / daysInMonth,
          Fat: totalNutrisi.Fat / daysInMonth,
          Carbohydrates: totalNutrisi.Carbohydrates / daysInMonth,
          Fiber: totalNutrisi.Fiber / daysInMonth,
          Sugar: totalNutrisi.Sugar / daysInMonth,
          Sodium: totalNutrisi.Sodium / daysInMonth,
        };

        setTotalGizi(avgNutrisi);
        ...
        const inputData = {
          ...
          Calories: [Math.max(0, avgNutrisi.Calories || 0)],
          Protein: [Math.max(0, avgNutrisi.Protein || 0)],
          Fat: [Math.max(0, avgNutrisi.Fat || 0)],
          Carbohydrates: [Math.max(0, avgNutrisi.Carbohydrates || 0)],
          Fiber: [Math.max(0, avgNutrisi.Fiber || 0)],
          Sugar: [Math.max(0, avgNutrisi.Sugar || 0)],
          Sodium: [Math.max(0, avgNutrisi.Sodium || 0)],
        };
      ...
      <h2 className="text-base sm:text-lg md:text-xl font-bold text-main mb-4 sm:mb-6 md:mb-8 flex items-center animate-float">
        ...
        <div>
          ...
          <div className="text-xs sm:text-xs md:text-sm text-gray-500 mt-0.5 sm:mt-1">
            Berdasarkan pola makan bulan ini
          </div>
        </div>
      </h2>
              ...
              <div className="flex items-start gap-3 sm:gap-4">
                ...
                <div className="flex-1">
                  <p className="text-xs sm:text-sm md:text-base text-gray-600 leading-relaxed">
                    Prediksi ini berdasarkan rata-rata nutrisi harian dari
                    makanan yang Anda konsumsi selama seminggu terakhir. Hasil
                    dapat berubah sesuai dengan pola makan Anda.
                  </p>
...
export default MonthlyDiseasePrediction;
