...
const MonthlyAdditionalRecommendations = () => {
  ...
  const analyzeNutrition = React.useCallback(async () => {
    ...
    try {
      ...
      // Mengambil riwayat scan bulan ini
      ...
      const totalNutrisi = monthlyHistory.reduce((acc, item) => {
      ...
      // Menghitung target bulanan
      const targetBulanan = {
        energi: targetHarian.energi * daysInMonth,
        protein: targetHarian.protein * daysInMonth,
        lemak_total: targetHarian["lemak total"] * daysInMonth,
        karbohidrat: targetHarian.karbohidrat * daysInMonth,
        serat: targetHarian.serat * daysInMonth,
        gula: targetHarian.gula * daysInMonth,
        garam: targetHarian.garam * daysInMonth,
      };

      // Menganalisis nutrisi yang berlebihan
      const recommendations = [];

      if (totalNutrisi.energi > targetBulanan.energi) {
        recommendations.push({
          nutrient: "Energi",
          current: Math.round(totalNutrisi.energi),
          target: Math.round(targetBulanan.energi),
          excess: Math.round(totalNutrisi.energi - targetBulanan.energi),
          recommendations: [
            "Buat jadwal olahraga rutin",
            "Kurangi porsi kalori harian",
            "Konsultasi dengan ahli gizi",
          ],
        });
      }

      if (totalNutrisi.protein > targetBulanan.protein) {
        recommendations.push({
          nutrient: "Protein",
          current: Math.round(totalNutrisi.protein),
          target: Math.round(targetBulanan.protein),
          excess: Math.round(totalNutrisi.protein - targetBulanan.protein),
          recommendations: [
            "Evaluasi sumber protein",
            "Tambah porsi sayuran",
            "Kurangi suplemen protein",
          ],
        });
      }

      if (totalNutrisi.lemak_total > targetBulanan.lemak_total) {
        recommendations.push({
          nutrient: "Lemak",
          current: Math.round(totalNutrisi.lemak_total),
          target: Math.round(targetBulanan.lemak_total),
          excess: Math.round(
            totalNutrisi.lemak_total - targetBulanan.lemak_total
          ),
          recommendations: [
      ...
      if (totalNutrisi.karbohidrat > targetBulanan.karbohidrat) {
        recommendations.push({
          nutrient: "Karbohidrat",
          current: Math.round(totalNutrisi.karbohidrat),
          target: Math.round(targetBulanan.karbohidrat),
          excess: Math.round(
            totalNutrisi.karbohidrat - targetBulanan.karbohidrat
          ),
          recommendations: [
            "Atur porsi nasi/roti",
      ...
      if (totalNutrisi.gula > targetBulanan.gula) {
        recommendations.push({
          nutrient: "Gula",
          current: Math.round(totalNutrisi.gula),
          target: Math.round(targetBulanan.gula),
          excess: Math.round(totalNutrisi.gula - targetBulanan.gula),
          recommendations: [
      ...
      if (totalNutrisi.garam > targetBulanan.garam) {
        recommendations.push({
          nutrient: "Garam",
          current: Math.round(totalNutrisi.garam),
          target: Math.round(targetBulanan.garam),
          excess: Math.round(totalNutrisi.garam - targetBulanan.garam),
          recommendations: [
            "Kurangi garam masakan",
            "Pakai bumbu alami",
  ...
  return (
    <div className="bg-gradient-to-br from-red-50 to-red-100/50 p-4 md:p-6 rounded-xl border border-red-200 shadow-sm hover:shadow-lg transition-all duration-300">
      <h2 className="text-lg md:text-xl font-bold text-red-800 mb-4 flex items-center gap-2">
        ...
        Rekomendasi Penyesuaian Nutrisi Bulanan
      </h2>
...
export default MonthlyAdditionalRecommendations;
