...
const InstructionsSection = () => {
  ...
  return (
    <div className="bg-white/90 rounded-xl sm:rounded-2xl p-3 sm:p-4 md:p-8 shadow-lg backdrop-blur-md">
      {/* Header Section */}
      <div className="mb-4 sm:mb-6 md:mb-8 text-center">
        <h2 className="text-lg sm:text-xl md:text-2xl font-bold text-main mb-2 flex items-center justify-center gap-1.5 sm:gap-2 md:gap-3">
          <svg
            xmlns="http://www.w3.org/2000/svg"
            className="h-5 w-5 sm:h-6 sm:w-6 md:h-8 md:w-8 text-secondary"
            fill="none"
            viewBox="0 0 24 24"
            stroke="currentColor"
          >
      ...
      <div className="grid lg:grid-cols-2 gap-4 md:gap-8">
        {/* Upload Section */}{" "}
        <div className="bg-gradient-to-br from-main/5 to-secondary/10 rounded-lg sm:rounded-xl p-3 sm:p-4 md:p-6 transform transition-all duration-300 hover:scale-[1.02]">
          <div className="flex items-center gap-2 sm:gap-3 mb-4 sm:mb-6">
            <div className="p-2 sm:p-3 bg-secondary/10 rounded-full">
              <svg
                xmlns="http://www.w3.org/2000/svg"
                className="h-4 w-4 sm:h-5 sm:w-5 md:h-6 md:w-6 text-main"
                fill="none"
                viewBox="0 0 24 24"
                stroke="currentColor"
              >
          ...
          <div className="space-y-2 sm:space-y-4">
            {uploadSteps.map((step, index) => (
              <div
                key={index}
                className="flex gap-2 sm:gap-4 items-start bg-white/80 p-2.5 sm:p-4 rounded-lg shadow-sm hover:shadow-md transition-all duration-300"
              >
                <div className="flex-shrink-0 w-6 h-6 sm:w-8 sm:h-8 bg-main text-white rounded-full flex items-center justify-center text-sm sm:text-base font-bold">
                  {index + 1}
                </div>
                <div>
                  <h4 className="font-medium text-main mb-0.5 sm:mb-1 text-sm sm:text-base">
                    {step.title}
                  </h4>
      ...
      {/* Tips Section */}
      <div className="mt-4 sm:mt-6 md:mt-8 p-2.5 sm:p-3 md:p-4 bg-secondary/5 rounded-lg">
        <p className="text-[10px] sm:text-xs md:text-sm text-main/80 text-center">
